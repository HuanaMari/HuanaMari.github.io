function Kalkulator() {
    ...
    this.createInput = () => {
        ...
        checkForNumbers =(selector)=>{
             var children = $(selector).children('input[type=text]');
              var regex=/^[a-zA-Z]+$/;
             for(var i = 0;i<children.length;i++){
                var boolean = true
            if ($(children[i]).val().match(regex))
            {
        ...
        kalkulatorO = () => {
            ...
            $(lengthInput).keypress( ()=> {
                // if (String.fromCharCode(e.keyCode).match(/[^0-9]/g)) return false;
                // var inputVal = $('.inputs');
                // checkForNumbers(inputVal);
    ...
    this.displayInput = () => {
        ...
        $(document).on('keypress', (event) => {
            ...
            if (event.which == 13) {
                if (searchedCity !== '') {
                    // if ( this.displayWeatherDetails()) {
                        console.log(this.displayWeatherDetails())
                        $('.weatherDiv').html(' ');
